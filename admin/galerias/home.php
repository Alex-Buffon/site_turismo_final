...
<div class="content-header">
    <h2>Galeria da Página Principal</h2>
...
<div class="content-body">
    <div class="galeria-grid">
        <?php
        $stmt = $pdo->query("SELECT * FROM galerias WHERE tipo = 'home' ORDER BY id DESC");
        while ($imagem = $stmt->fetch()) {
        ?>
            <div class="galeria-item">
                <img src="../uploads/galerias/<?php echo $imagem['imagem']; ?>" alt="<?php echo $imagem['titulo']; ?>">
                <div class="galeria-info">
                    <h4><?php echo $imagem['titulo']; ?></h4>
                    <p><?php echo $imagem['descricao']; ?></p>
                    <div class="galeria-acoes">
                        <button class="btn btn-sm btn-info" onclick="editarImagem(<?php echo $imagem['id']; ?>)">Editar</button>
                        <button class="btn btn-sm btn-danger" onclick="excluirImagem(<?php echo $imagem['id']; ?>)">Excluir</button>
                    </div>
                </div>
            </div>
        <?php } ?>
...
<div class="modal" id="addImagem">
    <div class="modal-content">
        ...
        <form method="POST" enctype="multipart/form-data" id="formAddImagem">
            <div class="form-group">
                <label>Título:</label>
